// Full-site crawling. Discovers and fetches multiple pages on a domain,
// extracting rich per-page data for each one, and layers on site-wide
// analysis that only makes sense once more than one page is known: orphan
// pages, weak internal linking, duplicate content, and recurring technical
// issues.
//
// This is the ONLY way a page's data enters VertexRank — there is no
// separate single-page scan route. The per-page extraction and issue rules
// below (detectPageIssues) are applied identically to every crawled page, so
// results are consistent across the whole site.
//
...
/** Extracts a rich per-page data object (title, meta description, headings,
 * schema, etc.), plus the actual internal link URLs — needed for site-wide
 * orphan/weak-link detection, which only makes sense across multiple
 * crawled pages. */
...
/** Deterministic issue rules, applied to one crawled page. Site-wide Audit
 * groups these across every crawled page into one entry per problem, with
 * every affected URL. */
