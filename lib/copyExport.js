...
export function buildUnifiedRecs(b, limit = Infinity) {
  const list = [];
  const siteKw = arr(b?.siteKeywordClusters?.opportunities);
  const kwOpps = siteKw.length ? siteKw : arr(b?.keywordIntel?.opportunities);
  kwOpps.forEach((o) => list.push({
    id: `uni_kw_${o.id}`, module: siteKw.length ? "Site-wide Keyword Clustering" : "Keyword Intelligence",
    title: o.problem || o.opportunity, evidence: o.evidence,
    recommendedAction: o.recommendedAction || o.opportunity, priority: o.priority, confidence: o.confidence || "Medium", source: o.source, actionType: "Content Update",
  }));
  const siteAeo = arr(b?.siteAeoAnalysis?.contentRecommendations);
  const aeoRecs = siteAeo.length ? siteAeo : arr(b?.aeoAnalysis?.contentRecommendations);
  aeoRecs.forEach((r) => list.push({
    id: `uni_aeo_${r.id}`, module: siteAeo.length ? "Site-wide AEO" : "AEO", title: r.recommendation, evidence: r.why,
    recommendedAction: r.recommendation, priority: r.priority, confidence: "Medium", source: r.source, actionType: "FAQ Addition",
  }));
  const siteGeo = arr(b?.siteGeoAnalysis?.opportunities);
  const geoOpps = siteGeo.length ? siteGeo : arr(b?.geoAnalysis?.opportunities);
  geoOpps.forEach((o) => list.push({
    id: `uni_geo_${o.id}`, module: siteGeo.length ? "Site-wide GEO" : "GEO / AI Visibility", title: o.title, evidence: o.why,
