...
function Dashboard() {
  ...
  // already produced by Keyword Intelligence, AEO, and GEO into one
  // prioritized list, each traceable back to its module and evidence.
  ...
  const unifiedRecs = useMemo(() => buildUnifiedRecs(b, 8), [b.keywordIntel, b.aeoAnalysis, b.geoAnalysis]);

  if (!currentWebsiteId) {
    return <EmptyState icon={Globe2} title="No website yet" body="Add a website, then run a scan to get a real technical and on-page SEO audit." action={<Button icon={Plus} onClick={() => setView("scanner")}>Add a website</Button>} />;
  }
  const scores = websites.find((w) => w.id === currentWebsiteId)?.scores || {};
  const scanned = !!b.scanMeta?.live;
  const critical = b.issues.filter((i) => i.severity === "Critical" || i.severity === "High").slice(0, 4);
  const quickWins = b.issues.filter((i) => i.severity === "Low" || i.severity === "Medium").slice(0, 4);
    ...
    <div className="space-y-6">
      {scanned ? (
        <div className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-xs" style={{ background: BRAND.visibilitySoft, color: BRAND.visibility }}>
          ...
          Technical and Content scores are from a live scan of this URL (HTTP {b.scanMeta.statusCode}, {fmtDate(b.scanMeta.fetchedAt)}). AEO, GEO, and Keyword scores are VertexRank AI estimates — run each analysis from its module to compute them.
        </div>
      ) : (
        <div className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-xs" style={{ background: BRAND.amberSoft, color: BRAND.amber }}>
          <Info size={14} />
          No scan has been run for this site yet. Go to Websites and click Start scan to pull a real Technical and Content audit.
        </div>
      )}
      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-center gap-6 justify-around">
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[10px] font-medium" style={{ color: BRAND.inkSoft }}>{scanned ? "From Technical + Content" : "Not scanned"}</span>
          </div>
          <div className="flex flex-col items-center gap-1">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[10px] font-medium" style={{ color: scanned ? BRAND.visibility : BRAND.inkSoft }}>{scanned ? "Live" : "Not scanned"}</span>
          </div>
          <div className="flex flex-col items-center gap-1">
      ...
      <Card className="p-5">
        <div className="flex items-center justify-between mb-1">
          ...
          {b.aiInsights && (
            <div className="flex items-center gap-1">
              <CopyButton label="Copy" title="Copy the AI Insights" getText={() => sectionAiInsights(b.aiInsights)} />
              <Button size="sm" variant="ghost" icon={RefreshCw} onClick={generateInsights} disabled={insightsLoading}>
        ...
        {!scanned ? (
          <p className="text-sm mt-2" style={{ color: BRAND.inkSoft }}>Run a scan first — VertexRank AI reads the real scan results to write about this specific site, not a generic template.</p>
        ) : insightsLoading ? (
          <div className="flex flex-col items-center py-8 gap-3">
            <Loader2 className="vr-spin" size={24} style={{ color: BRAND.primary }} />
            <p className="text-sm" style={{ color: BRAND.inkSoft }}>VertexRank AI is reading the site and writing insights…</p>
          </div>
        ) : !b.aiInsights ? (
          <div className="pt-2">
            <p className="text-sm mb-3" style={{ color: BRAND.inkSoft }}>VertexRank AI will read this site's real content and audit results, then write a plain-language summary, a few grounded quick wins, and strategic advice — specific to this site, not a template.</p>
          ...
          <div className="space-y-5 pt-3">
            <div>
              ...
              <p className="text-sm leading-relaxed">{b.aiInsights.summary}</p>
            </div>

            {b.aiInsights.quickWins?.length > 0 && (
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: BRAND.inkSoft }}>Grounded quick wins</p>
                <div className="space-y-3">
                  {b.aiInsights.quickWins.map((qw, idx) => (
            ...
            <div className="rounded-lg p-4" style={{ background: BRAND.primarySoft }}>
              ...
              <p className="text-sm leading-relaxed">{b.aiInsights.strategicInsight}</p>
            </div>

            <p className="text-[11px]" style={{ color: BRAND.inkSoft }}>Generated by VertexRank AI · {fmtDate(b.aiInsights.generatedAt)}</p>
            {insightsError && <p className="text-xs" style={{ color: BRAND.red }}>{insightsError}</p>}
          </div>
        )}
      </Card>
      ...
      <Card className="p-5">
        ...
        <p className="text-sm mb-4" style={{ color: BRAND.inkSoft }}>
          Every gap found by Keyword Intelligence, AEO, and GEO — reasoned together, ranked by priority, and one click from the Action Center.
        </p>
        {unifiedRecs.length === 0 ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="p-5 lg:col-span-2">
          ...
          {scanned ? (
            ...
            <div className="h-[220px] flex items-center justify-center text-sm" style={{ color: BRAND.inkSoft }}>Run a scan to see this chart.</div>
          )}
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="p-5">
          ...
          <div className="space-y-2.5">
            {critical.length === 0 && (
              <p className="text-sm" style={{ color: BRAND.inkSoft }}>
                {scanned ? "Nothing critical outstanding — nice work." : "Run a scan to see issues here."}
              </p>
            )}
            {critical.map((i) => (
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2.5">
            {quickWins.length === 0 && (
              <p className="text-sm" style={{ color: BRAND.inkSoft }}>
                {scanned ? "No quick wins outstanding." : "Run a scan to see issues here."}
              </p>
            )}
            {quickWins.map((i) => (
              <div key={i.id} className="flex items-center justify-between gap-2 rounded-lg px-3 py-2" style={{ background: BRAND.canvas }}>
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2.5">
            {b.positives.length === 0 && (
              <p className="text-sm" style={{ color: BRAND.inkSoft }}>
                {scanned ? "Nothing passing yet — see the issues list." : "Run a scan to see what's already working."}
              </p>
            )}
...
function ActionCenter() {
  ...
  function openAdd() { setForm({ title: "", type: ACTION_TYPES[0], status: "New" }); setModal({ mode: "add" }); }
  function openEdit(a) { setForm({ title: a.title, type: a.type, status: a.status }); setModal({ mode: "edit", action: a }); }
  function save() {
    if (!form.title.trim()) return;
    if (modal.mode === "add") {
      mutate((arr) => [{ id: uid("act"), websiteId: currentWebsiteId, createdAt: new Date().toISOString(), ...form }, ...arr]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center justify-between">
        ...
        <Button icon={Plus} onClick={openAdd}>New action</Button>
      </div>

      <Card className="overflow-hidden">
        {actions.length === 0 ? <EmptyState icon={ClipboardCheck} title="No actions" body="Actions generated from audit issues, or created manually, will appear here." action={<Button icon={Plus} onClick={openAdd}>New action</Button>} /> : (
          <>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "New action" : "Edit action"}
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 rounded-lg p-1 w-fit" style={{ background: BRAND.canvas }}>
        {[["ai", "AI Keyword Intelligence"], ["clusters", "Site-wide Clustering"], ["manual", "Manual Tracking"]].map(([id, label]) => (
...
function ManualKeywordTracker() {
  ...
  async function suggestWithAi() {
    if (!b.scanMeta?.live) { toast("Run a live scan first, from Websites.", "error"); return; }
    setSuggesting(true);
    try {
      let intel = b.keywordIntel;
      if (!intel) {
        const data = await postJson("/api/keyword-intelligence", { scanMeta: b.scanMeta, targetKeyword: "", topic: "", country: COUNTRIES[0], language: "English" });
        intel = data.result;
        setBundles((prev) => {
          const site = prev[currentWebsiteId] || emptyBundle();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-xs" style={{ background: BRAND.amberSoft, color: BRAND.amber }}>
        <Info size={14} />
        Search volume, difficulty, and opportunity require a paid keyword data provider — not connected yet. Track the keywords and rankings you already know here; those fields will show "—" until a provider is connected.
      </div>
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <div className="flex flex-wrap gap-2">
          <SearchInput value={search} onChange={setSearch} placeholder="Search keywords..." />
...
function AeoView() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 rounded-lg p-1 w-fit" style={{ background: BRAND.canvas }}>
        {[["ai", "AI Answer Coverage"], ["site", "Site-wide AEO"], ["manual", "Manual Tracking"]].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} className="vr-focus text-sm font-medium rounded-md px-3 py-1.5"
            style={{ background: tab === id ? BRAND.surface : "transparent", color: tab === id ? BRAND.primary : BRAND.inkSoft, boxShadow: tab === id ? `0 1px 2px rgba(0,0,0,0.06)` : "none" }}>
            {label}
          </button>
...
function ManualAeoTracker() {
  ...
  function openAdd() { setForm({ question: "", hasAnswer: false }); setModal({ mode: "add" }); }
  function openEdit(q) { setForm({ question: q.question, hasAnswer: q.hasAnswer }); setModal({ mode: "edit", q }); }
  function save() {
    if (!form.question.trim()) return;
    if (modal.mode === "add") {
      mutate((arr) => [...arr, { id: uid("aeo"), websiteId: currentWebsiteId, question: form.question.trim(), hasAnswer: form.hasAnswer }]);
  ...
  // AI Answer Coverage tab) instead of requiring the user to type each one.
  async function suggestWithAi() {
    if (!b.scanMeta?.live) { toast("Run a live scan first, from Websites.", "error"); return; }
    setSuggesting(true);
    try {
      let aeo = b.aeoAnalysis;
      if (!aeo) {
        const data = await postJson("/api/aeo-analysis", { scanMeta: b.scanMeta, keywords: b.keywordIntel?.keywords || [] });
        aeo = data.result;
        setBundles((prev) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 rounded-lg px-4 py-2.5 text-xs" style={{ background: BRAND.amberSoft, color: BRAND.amber }}>
        <Info size={14} />
        There's no free, automated way to discover what real people actually ask AI assistants about your site — that needs an AI-answer-tracking provider. But VertexRank AI can generate realistic candidate questions from your crawl (via "Suggest with AI"), or add the ones you already know about by hand.
      </div>
      <div className="flex justify-end gap-2">
        <Button variant="soft" icon={suggesting ? Loader2 : Sparkles} disabled={suggesting} onClick={suggestWithAi}>{suggesting ? "Suggesting…" : "Suggest with AI"}</Button>
        <Button icon={Plus} onClick={openAdd}>Add question</Button>
      </div>

      {b.aeoQuestions.length === 0 ? (
        <Card><EmptyState icon={MessageCircleQuestion} title="No questions tracked yet" body="Let VertexRank AI suggest questions from the crawl, or add a question a buyer might ask by hand." action={<div className="flex gap-2"><Button variant="soft" icon={Sparkles} onClick={suggestWithAi}>Suggest with AI</Button><Button icon={Plus} onClick={openAdd}>Add question</Button></div>} /></Card>
      ) : (
        <>
          <Card className="p-5">
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "Add question" : "Edit question"}
        footer={<><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></>}>
...
function GeoView() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 rounded-lg p-1 w-fit" style={{ background: BRAND.canvas }}>
        {[["geo", "GEO / AI Visibility"], ["site", "Site-wide GEO"], ["sim", "AI Visibility Simulator"]].map(([id, label]) => (
...
function CompetitorsView() {
  ...
  const [modal, setModal] = useState(null);
  const [confirmDelete, setConfirmDelete] = useState(null);
  const [form, setForm] = useState({ name: "", url: "", seoScore: "", sharedKeywords: "", gaps: "" });
  ...
  function openAdd() { setForm({ name: "", url: "", seoScore: "", sharedKeywords: "", gaps: "" }); setModal({ mode: "add" }); }
  function openEdit(c) { setForm({ name: c.name, url: c.url, seoScore: c.seoScore ?? "", sharedKeywords: c.sharedKeywords ?? "", gaps: (c.gaps || []).join(", ") }); setModal({ mode: "edit", c }); }
  function save() {
    if (!form.name.trim()) return;
    const payload = {
      name: form.name.trim(),
      url: form.url.trim() || form.name.toLowerCase().replace(/\s+/g, "") + ".com",
      seoScore: form.seoScore ? Number(form.seoScore) : null,
      sharedKeywords: form.sharedKeywords ? Number(form.sharedKeywords) : null,
      gaps: form.gaps.split(",").map((g) => g.trim()).filter(Boolean),
    };
    if (modal.mode === "add") {
      mutate((arr) => [...arr, { id: uid("comp"), websiteId: currentWebsiteId, analysis: null, ...payload }]);
  ...
  async function suggestCompetitors() {
    ...
    setSuggesting(true);
    try {
      const data = await postJson("/api/competitor-suggestions", { scanMeta: b.scanMeta });
  ...
  function acceptSuggestion(s) {
    mutate((arr) => [...arr, { id: uid("comp"), websiteId: currentWebsiteId, analysis: null, name: s.name, url: s.url || s.name.toLowerCase().replace(/\s+/g, "") + ".com", seoScore: null, sharedKeywords: null, gaps: [] }]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button icon={Plus} onClick={openAdd}>Add competitor</Button>
      </div>
      ...
      {b.competitors.length === 0 ? <Card><EmptyState icon={Users} title="No competitors tracked" body="Let VertexRank AI suggest competitors from the crawl, or add one you already know by hand." action={<div className="flex gap-2"><Button variant="soft" icon={Sparkles} onClick={suggestCompetitors}>Suggest with AI</Button><Button icon={Plus} onClick={openAdd}>Add competitor</Button></div>} /></Card> : (
          ...
          {b.competitors.map((c) => (
            <Card key={c.id} className="p-4">
              ...
              <div className="flex items-center gap-4 mt-3">
                <div><p className="vr-mono font-semibold text-lg" style={{ color: c.seoScore == null ? BRAND.inkSoft : BRAND.ink }}>{c.seoScore ?? "—"}</p><p className="text-xs" style={{ color: BRAND.inkSoft }}>SEO score</p></div>
                <div><p className="vr-mono font-semibold text-lg" style={{ color: c.sharedKeywords == null ? BRAND.inkSoft : BRAND.ink }}>{c.sharedKeywords ?? "—"}</p><p className="text-xs" style={{ color: BRAND.inkSoft }}>Shared keywords</p></div>
              </div>
              {c.gaps && c.gaps.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-semibold mb-1.5" style={{ color: BRAND.inkSoft }}>Content gaps vs. you</p>
                  <div className="flex flex-wrap gap-1.5">{c.gaps.map((g) => <span key={g} className="text-xs rounded px-2 py-1" style={{ background: BRAND.amberSoft, color: BRAND.amber }}>{g}</span>)}</div>
                </div>
              )}
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "Add competitor" : "Edit competitor"}
        footer={<><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></>}>
        ...
        <Field label="SEO score (optional, if you know it)"><TextInput type="number" min="0" max="100" value={form.seoScore} onChange={(e) => setForm((f) => ({ ...f, seoScore: e.target.value }))} /></Field>
        <Field label="Shared keywords (optional, if you know it)"><TextInput type="number" min="0" value={form.sharedKeywords} onChange={(e) => setForm((f) => ({ ...f, sharedKeywords: e.target.value }))} /></Field>
        <Field label="Content gaps vs. you (comma separated)"><TextInput placeholder="Pricing page, case studies" value={form.gaps} onChange={(e) => setForm((f) => ({ ...f, gaps: e.target.value }))} /></Field>
      </Modal>
...
function ContentPlannerView() {
  ...
  function openAdd() { setForm(blankForm); setModal({ mode: "add" }); }
  function openEdit(c) {
    setForm({
  ...
  function save() {
    ...
    if (modal.mode === "add") {
      mutate((arr) => [...arr, { id: uid("ci"), websiteId: currentWebsiteId, internalLinks: 0, ...payload }]);
      toast("Content idea created");
    } else {
      mutate((arr) => arr.map((x) => (x.id === modal.c.id ? { ...x, ...payload } : x)));
  ...
  // Keyword Intelligence, AEO, and GEO, so the user never has to type a
  // title before VertexRank AI can help.
  async function generateIdeasWithAi() {
    if (!b.scanMeta?.live) { toast("Run a live scan first, from Websites.", "error"); return; }
    setGeneratingIdeas(true);
    try {
      const data = await postJson("/api/content-ideas", {
        scanMeta: b.scanMeta,
        keywordOpportunities: b.keywordIntel?.opportunities || [],
        aeoQuestions: b.aeoAnalysis?.questions || [],
        geoOpportunities: b.geoAnalysis?.opportunities || [],
        count: 4,
      });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end gap-2">
        ...
        <Button icon={Plus} onClick={openAdd}>New content idea</Button>
      </div>
      {b.contentIdeas.length === 0 ? <Card><EmptyState icon={FileText} title="No content ideas yet" body="Let VertexRank AI generate complete ideas from the crawl and known gaps, or plan one yourself." action={<div className="flex gap-2"><Button variant="soft" icon={Sparkles} onClick={generateIdeasWithAi}>Generate ideas with AI</Button><Button icon={Plus} onClick={openAdd}>New content idea</Button></div>} /></Card> : (
        <div className="grid sm:grid-cols-2 gap-4">
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "New content idea" : "Edit content idea"} width={620}
        footer={<><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></>}>
        <Field label="Title"><TextInput value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} /></Field>
...
function InternalLinksView() {
  ...
  function openAdd() { setForm({ source: "", target: "", anchor: "", reason: "" }); setModal({ mode: "add" }); }
  ...
  function save() {
    ...
    if (modal.mode === "add") { mutate((arr) => [...arr, { id: uid("lnk"), websiteId: currentWebsiteId, status: "Suggested", ...form }]); toast("Link opportunity added"); }
    else { mutate((arr) => arr.map((x) => (x.id === modal.l.id ? { ...x, ...form } : x))); toast("Link opportunity updated"); }
    setModal(null);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end gap-2"><Button icon={Plus} onClick={openAdd}>Add link opportunity</Button></div>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "Add link opportunity" : "Edit link opportunity"}
        footer={<><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></>}>
...
function BacklinksView() {
  ...
  function save() {
    if (!form.name.trim()) return;
    if (modal.mode === "add") {
      mutate((arr) => [{ id: uid("bl"), websiteId: currentWebsiteId, ...form }, ...arr]);
      toast("Backlink opportunity added");
    } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between">
        ...
        <Button icon={Plus} onClick={openAdd}>Add opportunity</Button>
      </div>
      {b.backlinks.length === 0 ? (
        <Card><EmptyState icon={Rss} title="No backlink opportunities tracked yet" body="Add prospects from the discovery tools above, or add one you already know by hand." action={<Button icon={Plus} onClick={openAdd}>Add opportunity</Button>} /></Card>
      ) : (
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.mode === "add" ? "Add backlink opportunity" : "Edit backlink opportunity"}
        footer={<><Button variant="outline" onClick={() => setModal(null)}>Cancel</Button><Button onClick={save}>Save</Button></>}>
